    ...
    try:
        ...
        for tk in tickers:
            try:
                ...
                if df5.empty:
                    continue
                # avg_bar_volume: daily volume / (number of 5-min bars in session ~78)
                if not df1d.empty and "Volume" in df1d.columns:
                    avg_daily_vol = float(df1d["Volume"].iloc[:-1].mean())
                    avg_bar_vol   = avg_daily_vol / 78.0
                else:
                    avg_bar_vol = float(df5["Volume"].mean()) if "Volume" in df5.columns else 0
...
def build_journal_section(nav_history: dict, idx_returns: dict) -> str:
    ...
    For intraday, P&L is realized closed-trade PnL per day (not mark-to-market)."""
    if not nav_history:
        return ""
      ...
      <table>
        <thead>
          <tr>
            ...
            <th colspan="3" style="background:#bf360c;text-align:center;font-size:11px">Your Portfolio</th>
            <th colspan="4" style="background:#bf360c;text-align:center;font-size:11px">Index Returns</th>
...
(function() {{
  ...
  const vals = data.values, N = vals.length;
  if (N < 2) return;
  ...
  ctx.fillText(lastVal, W - pad - ctx.measureText(lastVal).width, 18);
}})();
</script>
</body>
</html>"""
