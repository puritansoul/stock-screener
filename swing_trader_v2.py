"""
Swing Trading Bot v2.0 — RSI(2) Mean Reversion + Enhanced Filters

Rules (Larry Connors + v2 improvements):
  ENTRY long : close > 200-day SMA  AND  RSI(2) < 5  AND  VIX >= 20
               AND  sector ETF > SMA(200)  AND  no earnings within 3 days
  EXIT long  : RSI(2) > 65  (or stop, or 7-day time exit)
  ENTRY short: close < 200-day SMA  AND  RSI(2) > 95  AND  SPY < SPY SMA200
               AND  VIX >= 20  AND  no earnings within 3 days
  EXIT short : RSI(2) < 35  (or stop, or 7-day time exit)

v2 improvements over v1:
  - VIX >= 20 entry gate (elevated vol strengthens RSI(2) edge)
  - Earnings blackout: skip entry within 3 days of next earnings
  - ATR-based position sizing tiers (scale down extremes)
  - 7-day calendar time-based exit (~5 trading days)
  - Sector ETF SMA200 relative strength filter for longs
...
Runs     : daily at 10:20am ET via GitHub Actions (swing_trading_v2.yml)
State    : swing_trades_v2.json  (all open + closed positions + NAV curve)
"""
...
def position_size(capital: float, entry: float, stop: float, atr_val: float = 0.0) -> int:
    """Fixed-fraction risk: risk 2% of capital between entry and stop.
    v2: ATR-based tier scaling — scale down extreme-vol stocks."""
    risk_dollars = capital * RISK_PER_TRADE
    per_share_risk = abs(entry - stop)
    if per_share_risk < 0.01:
        return 0
    shares = int(risk_dollars / per_share_risk)
    # ATR-based tier: mid-volatility gets full size; extremes get scaled
    if atr_val > 0 and entry > 0:
        atr_pct = atr_val / entry
        if atr_pct < 0.01:
            shares = int(shares * 0.5)    # too tight/noisy
        elif atr_pct > 0.03:
            shares = int(shares * 0.75)   # too volatile
...
def fetch_vix() -> float:
    """Return latest VIX close, or 0.0 on failure."""
    try:
...
def fetch_sector_sma200() -> dict[str, bool]:
    """Return {sector_name: is_above_sma200} for each sector ETF."""
    etfs = list(SECTOR_ETFS.values())
    result = {}
    try:
        raw = yf.download(etfs, period="220d", auto_adjust=True, progress=False)
        if raw.empty:
            return {}
        close = raw["Close"] if isinstance(raw.columns, pd.MultiIndex) else raw
        for sector, etf in SECTOR_ETFS.items():
            if etf not in close.columns:
                continue
            s = close[etf].dropna()
            if len(s) < SMA200_PERIOD:
                continue
            sma = float(s.rolling(SMA200_PERIOD).mean().iloc[-1])
            cur = float(s.iloc[-1])
            result[sector] = cur > sma
    except Exception:
        pass
    return result
...
def get_ticker_sector(ticker: str) -> str:
    """Return GICS sector string for a ticker (cached), or empty string on failure."""
...
def has_earnings_soon(ticker: str, days: int = EARNINGS_BLACKOUT) -> bool:
    """Return True if ticker has earnings within the next `days` calendar days."""
    try:
...
def run_swing_trader():
    ...
    for pos in open_pos:
        ...
        exit_reason = None
        # v2: time-based exit after TIME_EXIT_DAYS calendar days
    ...
    for tk in universe[:300]:
        ...
        if long_signal and entries_gate:
            # v2: sector ETF filter for longs
            sec = get_ticker_sector(tk)
            if sec and sec in sector_above_sma200 and not sector_above_sma200[sec]:
                status = f"{trend} (sector ETF below SMA200)"
                all_scan[-1]["status"] = status
            else:
                candidates.append({"ticker": tk, "side": "long",  "rsi2": rsi2, "close": close, "atr": atr_val})
        elif short_signal and entries_gate:
    ...
    if slots_available > 0:
        ...
        for cand in ordered:
            ...
            # v2: earnings blackout filter
            if has_earnings_soon(tk):
                log_entry["notes"].append(f"SKIP {tk}: earnings within {EARNINGS_BLACKOUT}d")
                print(f"  SKIP {tk}: earnings within {EARNINGS_BLACKOUT}d")
                continue

            stop = entry - ATR_STOP_MULT * atr_v if side == "long" else entry + ATR_STOP_MULT * atr_v
            shares = position_size(capital, entry, stop, atr_v)  # v2: pass atr for tier scaling
            if shares <= 0:
                continue
...
<head>
  ...
  <title>Swing Trader v2.0 — {today_str}</title>
  <style>
...
<body>
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px">
    <div>
      <h1 style="margin-bottom:4px">📈 Swing Trader v2.0 — RSI(2) + Enhanced Filters</h1>
      <p style="margin: 6px 0 12px">
        <span class="badge">{today_str}</span>&nbsp;
        <span class="badge">RSI(2) + VIX Gate</span>&nbsp;
...
(function() {{
  document.querySelectorAll('details').forEach(function(el) {{
    ...
    out = report_dir / f"swing_v2_{date.today().isoformat()}.html"
    out.write_text(html)
