"""
Live Multi-Factor S&P 500 Monitor & Rebalance Notifier — v2.0

v2 improvements over v1:
  - Top 5% concentration (was 10%) — stronger factor bets, fewer diluted positions
  - 25% sector cap — prevents over-concentration in any one sector
  - Piotroski F-score minimum raised to 6 (was 5) — higher quality bar
  - Size factor tilt — small-cap bias within the large-cap universe
  - Separate state files: portfolio_state_v2.json / portfolio_nav_v2.json
  - Reports saved as reports/v2_{date}.html
...
- Runs at 10:15am weekdays via cron

Setup (run once):
  python3 live_monitor_v2.py --setup
"""
...
TOP_DECILE_PCT        = 0.05        # v2: top 5% = your holdings (concentrated)
...
STATE_FILE = BASE_DIR / "portfolio_state_v2.json"
NAV_FILE   = BASE_DIR / "portfolio_nav_v2.json"
...
F_MKTCAP_MIN      = 1e9
F_ROIC_MIN        = 0.08
F_PIOTROSKI_MIN   = 6   # v2: raised from 5 to tighten quality bar
F_DEBT_EBITDA_MAX = 3.5
F_EV_EBITDA_MAX   = 25.0

# Scoring weights — v2: reduced rev_growth by 0.05 to add size factor
WEIGHTS = {
    "momentum":    0.20,
    "roic":        0.20,
    "fcf_ev":      0.15,
    ...
    "accruals":    0.10,
    "eps_surprise":0.10,
    "rev_growth":  0.00,  # v2: replaced by size factor below
    "size":        0.05,  # v2: small-cap tilt within universe
}

# ── v2 enhancements ───────────────────────────────────────────────────────────
SECTOR_CAP_PCT = 0.25   # max 25% allocation per GICS sector
...
def score_universe(
    ...
    # v2: size factor — percentile rank of inverse market cap (small-cap tilt)
    mkt_cap_raw = price_snap * fund["shares"]
    inv_mktcap = (1.0 / mkt_cap_raw.replace(0, float("nan")))
    ranks["r_size"] = pct_rank(inv_mktcap, True)
    ...
    rank_weight_map = {
        ...
        "r_size":         WEIGHTS["size"],  # v2: size factor
    }
...
def compute_weights(
    ...
    # v2: sector cap — no single sector may exceed SECTOR_CAP_PCT of total weight
    try:
        sector_map = {}
        for t in held:
            try:
                sector_map[t] = yf.Ticker(t).info.get("sector", "Unknown") or "Unknown"
            except Exception:
                sector_map[t] = "Unknown"
        # Iteratively scale down over-weight sectors until all are within cap
        for _ in range(10):  # max 10 passes to converge
            sector_totals: dict[str, float] = {}
            for t in held:
                s = sector_map.get(t, "Unknown")
                sector_totals[s] = sector_totals.get(s, 0.0) + float(w_final.get(t, 0.0))
            over = {s: v for s, v in sector_totals.items() if v > SECTOR_CAP_PCT + 1e-6}
            if not over:
                break
            for s, total in over.items():
                scale = SECTOR_CAP_PCT / total
                for t in held:
                    if sector_map.get(t) == s:
                        w_final[t] = w_final[t] * scale
            total_f = w_final.sum()
            if total_f > 0:
                w_final = w_final / total_f
    except Exception:
        pass  # sector cap is best-effort; fall back to uncapped weights
...
def save_html_report(
    ...
    path      = REPORT_DIR / f"v2_{today.isoformat()}.html"
...
<head>
  ...
  <title>Factor Screener v2.0 — {today.isoformat()}</title>
  <style>
...
<body>
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px">
    <div>
      <h1 style="margin-bottom:4px">📊 Multi-Factor S&amp;P 500 Screener v2.0</h1>
