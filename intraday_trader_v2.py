...
EARNINGS_CACHE   = BASE_DIR / "earnings_cache.json"   # shared with v1
...
ORB_ATR_MIN_RATIO    = 0.20   # ORB width must be >= 20% of 20-day ATR
...
def fetch_intraday(tickers: list[str]) -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        # Filter to today's bars only — period="1d" can return yesterday's full-day
        # data if called early, causing stale prices to be used as today's entries.
...
def scan_entries(state: dict, data: dict[str, dict],
    ...
    spy_chg   = regime.get("spy_change_pct", 0.0)
    ...
    for tk in universe:
        ...
        rel_strength = stock_chg - spy_chg
        if side == "long":
        ...
        candidates.append({
            ...
            "atr20": atr20, "ema20": ema20, "has_catalyst": tk in catalysts,
        })
...
def fetch_spy_regime() -> dict:
    ...
    long_ok  = SPY is up on the day AND above intraday VWAP AND above 20 EMA.
    short_ok = SPY is down on the day AND below intraday VWAP AND below 20 EMA.
    On fetch failure returns permissive defaults so the bot keeps running.
    """
    try:
        today_date = date.today()
        ...
        if not raw1d.empty:
            if isinstance(raw1d.columns, pd.MultiIndex):
                spy_close_1d = raw1d["Close"].iloc[:, 0]
            else:
...
def fetch_earnings_catalysts(tickers: list[str]) -> set[str]:
    ...
    Results cached per day. Only called for signal candidates, not full universe.
    """
    today     = date.today()
...
def run_intraday():
    ...
    # v2: pre-compute gap, VWAP5, SPY regime and catalysts (only on active trading days)
